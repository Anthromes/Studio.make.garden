        ...
        return ( <
                ...
                } >
                {
                    ...
                    sections.map((item, i) => ( <
                        >
                        <
                        ResizableImage key = { item.id + item.media.id }
                        item = { item }
                    ...
                    />} {
                        editMode === EDIT_MODES.annotation &&
                            sections.map((item, i) => < CanvasImage key = { i }
                                item = { item }
                                zoomLevel = { zoom }
                                />)} <
                                /InnerArea> <
                                ...
                                {
                                    editMode === EDIT_MODES.annotation && ( <
                                        Comments items = { annotations.filter(a => !a.parent_id) }
                                        activePin = { activePin }
                                        editMode = { editMode }
                                ...
                                <
                                ActionsBar onUpload = { addSection }
                                editMode = { editMode }
                                onChangeCanvasMode = { onChangeCanvasMode }
                                onUploadViaUrl = {
                                ...
                                <
                                Zoom onClickPlus = {
                                    () => zoom < MAX_ZOOM_LEVEL && setZoom(zoom + 1) }
                                onClickMinus = {
                                    () => zoom > -MAX_ZOOM_LEVEL && setZoom(zoom - 1) }
                                zoomLevel = { zoom }
                                />
